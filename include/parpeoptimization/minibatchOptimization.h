 ...
 */
enum class interceptType { none, reduceStep, reduceStepAndRestart };

/**
 * @brief Problem definition for mini-batch optimization.
 *
 * This class provides cost function and training data for a mini-batch
 * optimizer. Data maybe be the actual data or just an index list referencing
 * the data, the cost function will operate on.
 */
template <typename T>
class MinibatchOptimizationProblem : public OptimizationProblem {
  public:
    ...
    MinibatchOptimizationProblem(
        std::unique_ptr<SummedGradientFunction<T>> costFun,
        std::unique_ptr<Logger> logger)
        : OptimizationProblem(costFun, logger) {}

    MinibatchOptimizationProblem(MinibatchOptimizationProblem const& other) =
        delete;
    ...
    SummedGradientFunction<T>* getGradientFunction() const {
        auto summedGradientFunction =
            dynamic_cast<SummedGradientFunction<T>*>(cost_fun_.get());
 ...
 *
 * The LearningRateUpdater provides the possibility to reduce the learning rate
 * per epoch and makes it possible to adapt the learning rate according to
 * success or failure of the ODE solver.
 */
class LearningRateUpdater {
  public:
     ...
     * @param LearningRateadaptionMode Type of interpolation between
     * startLearningRate and endLearningRate
     */
    LearningRateUpdater(
        int maxEpochs,
  ...
  private:
    ...
    /** If an optimization step is not successful, the learning rate, i.e., step
     * size, will be reduced by this factor */
...
class ParameterUpdater {
  public:
    ...
    virtual void updateParameters(
        ...
        gsl::span<double const> lowerBounds = gsl::span<double const>(),
        gsl::span<double const> upperBounds = gsl::span<double const>()) = 0;

    /** If ODE becomes non-integrable, the last step must be undone using this
     * method */
    virtual void undoLastStep() = 0;

    /** If the ODE is repeatedly non-integrable, a cold restart is performed
     * using this method */
...
class ParameterUpdaterVanilla : public ParameterUpdater {
  public:
    ...
    void updateParameters(
        ...
        gsl::span<double> parameters,
        gsl::span<double const> lowerBounds = gsl::span<double const>(),
        gsl::span<double const> upperBounds =
            gsl::span<double const>()) override;
...
class ParameterUpdaterRmsProp : public ParameterUpdater {
  ...
  private:
    /** Rate for memorizing gradient norms (between 0 and 1, high rates mean
     * long memory) */
    ...
    /** Memorized gradient norms (decaying average), one step back (if one step
     * must be undone) */
...
class ParameterUpdaterMomentum : public ParameterUpdater {
  ...
  private:
    ...
    /** Accumulated momentum (decaying average), one step back (if one step must
     * be undone) */
...
class ParameterUpdaterAdam : public ParameterUpdater {
  ...
  private:
    /** Rate for memorizing gradients (between 0 and 1, high rates mean long
     * memory) */
    ...
    /** Memorized gradients (decaying average), one step back (if one step must
     * be undone) */
 ...
 */
template <typename T>
std::vector<std::vector<T>> getBatches(gsl::span<T const> data, int batchSize) {
    int numBatches = ceil(static_cast<double>(data.size()) / batchSize);

    std::vector<std::vector<T>> batches(numBatches, std::vector<T>());
    int batchIdx = -1;
    for (int i = 0;
         static_cast<typename decltype(data)::index_type>(i) < data.size();
         ++i) {
        if (i % batchSize == 0) {
            ++batchIdx;
 ...
 * @return the norm
 */
double getVectorNorm(gsl::span<double const> v);
 ...
 */
std::vector<double>
getVectorDifference(gsl::span<double const> v, gsl::span<double const> w);

template <typename BATCH_ELEMENT> class MinibatchOptimizer {
  public:
    /**
    ...
    std::tuple<int, double, std::vector<double>> optimize(
        ...
        gsl::span<double const> lowerParameterBounds,
        gsl::span<double const> upperParameterBounds,
        ...
        if (reporter) {
            ...
            reporter->result_writer_->setLoggingEachFunctionEvaluation(
                false, true);
        ...
        for (int epoch = 0; epoch < maxEpochs; ++epoch) {
            auto epochLogger =
                logger.getChild(std::string("e") + std::to_string(epoch));
            ...
            for (int batchIdx = 0; (unsigned)batchIdx < batches.size();
                 ++batchIdx) {
                auto batchLogger = epochLogger->getChild(
                    std::string("b") + std::to_string(batchIdx));
                iteration++;
                ...
                std::stringstream ss;
                ss << ": Cost: " << cost << " |g|2: " << getVectorNorm(gradient)
                ...
                if (status == functionEvaluationFailure) {
                    // Check, if the interceptor should be used (should always
                    // be the case, except for study purpose...
                    if (interceptor > interceptType::none)
                        status = rescueInterceptor(
                            parameters,
                            oldParameters,
                            gradient,
                            oldGradient,
                    ...
                    if (status == functionEvaluationFailure)
                        return finish(
                            cost,
                            parameters,
                            minibatchExitStatus::invalidNumber,
                            reporter,
                            batchLogger.get());

                } else {
                    // Cost function evaluation was successful, so we can
                    // increase the step size
                    subsequentFails = std::max(subsequentFails - 1, 0);
                    learningRateUpdater->increaseLearningRate();

                    // Overwrite old parameters and old gradient, since they
                    // won't be needed any more
                    std::copy(
                        gradient.begin(), gradient.end(), oldGradient.begin());
                    std::copy(
                        parameters.begin(),
                        parameters.end(),
                        oldParameters.begin());
            ...
            if (getVectorNorm(gradient) <= gradientNormThreshold) {
                ...
                auto dataSpan =
                    std::vector<BATCH_ELEMENT>(data.cbegin(), data.cend());
                evaluate(
                    f,
                    parameters,
                    dataSpan,
                ...
                return finish(
                    cost,
                    parameters,
                    minibatchExitStatus::gradientNormConvergence,
                    reporter,
                    epochLogger.get());
            }
        }
        ...
        return finish(
            cost,
            parameters,
            minibatchExitStatus::maxEpochsExceeded,
            reporter,
            &logger);
    }
    ...
    FunctionEvaluationStatus evaluate(
        ...
        double cpuTime = 0.0;
        auto status =
            f.evaluate(parameters, datasets, cost, gradient, logger, &cpuTime);
    ...
    std::tuple<int, double, std::vector<double>> finish(
        double cost,
        ...
        if (logger) {
            ...
            case minibatchExitStatus::invalidNumber:
                logger->logmessage(
                    loglevel::error,
                    "Mini-batch cost function evaluation failed.");
                break;
            case minibatchExitStatus::gradientNormConvergence:
                logger->logmessage(
                    loglevel::info,
                    "Convergence: gradientNormThreshold reached.");
                break;
            case minibatchExitStatus::maxEpochsExceeded:
                logger->logmessage(
                    loglevel::info, "Number of epochs exceeded.");
            }
        }
     ...
     */
    FunctionEvaluationStatus rescueInterceptor(
        gsl::span<double> parameters,
        ...
        // debug output
        if (logger) {
            std::vector<double> firstDifference =
                getVectorDifference(parameters, oldParameters);
            std::stringstream first_ss;
            first_ss << " Interceptor is active! Former step size: "
                     << getVectorNorm(firstDifference) << std::endl;
        ...
        while (status == functionEvaluationFailure) {
            ...
            // If the objective function evaluation failed, we want to undo the
            // step
            ++subsequentFails;
            parameterUpdater->undoLastStep();
            std::copy(oldGradient.begin(), oldGradient.end(), gradient.begin());
            std::copy(
                oldParameters.begin(), oldParameters.end(), parameters.begin());

            // note the previous parameter step for debugging
            parDifference = getVectorDifference(parameters, oldParameters);

            // Check if there are NaNs in the parameter vector now (e.g., fail
            // at first iteration)
            if (std::any_of(parameters.begin(), parameters.end(), [](double d) {
                    return std::isnan(d);
                }))
                initialFail = true;
            ...
            if (initialFail ||
                (finalFail &&
                 interceptor != interceptType::reduceStepAndRestart)) {
                logger->logmessage(
                    loglevel::debug,
                    "Failure at initial point of optimization. Stopping.");
            ...
            if (finalFail) {
                 ...
                 */
                ss << "Final failure. Rescue interceptor could not recover "
                      "optimization run."
                   << std::endl;
                subsequentFails = 0;
                ...
                ss << "Failure. Reducing learning rate and retry..."
                   << std::endl;
            }
            ...
            ss << ": Interceptor, before new evaluation: " << std::endl
               ...
               << ", Failures: " << subsequentFails << std::endl;
            ...
            status = evaluate(
                f, parameters, datasets, cost, gradient, logger, reporter);
    ...
    void handleStep(
        ...
        std::function<double(double)> evalLineSearch = [&f,
                                                        &datasets,
                                                        iteration,
                                                        &parameters,
                                                        &oldParameters,
         ...
         *
         * First compute a naive step as if there was no line-search -> get
         * cost1 If cost1 > cost0: Try to improve -> get cost2 If also cost2 >
         * cost0: If only short line-search is desired:  take min(cost1, cost2)
         *     If more longer line-search is desired: step into
         * performLineSearch()
         * */
        ...
        if (cost1 <= cost) {
            logger->logmessage(
                loglevel::debug, " Line-Search: Step was good right away...");
            return;
        }
        ...
        std::vector<double> parDifference =
            getVectorDifference(parameters, oldParameters);
        ...
        if (dirGradient > 0) {
            ...
            parabola_ss << " Line-Search: Not a descent direction! "
                        << std::endl;
            ...
            parabola_ss << "   cost0: " << cost
                        << " (step length: " << getVectorNorm(parDifference)
                        << "),   cost1: " << cost1
                        << " (step length: " << stepLength
                        << "),   cost2: " << cost2
                        << " (step length: " << newStepLength << ") "
                        << std::endl;
            if (logger)
        ...
        // Debugging output
        if (logger) {
            line_ss << " Line-Search: two steps were done, results are: "
                    << std::endl
                    << "   cost0: " << cost
                    << " (step length: " << getVectorNorm(parDifference)
                    << "),   cost1: " << cost1
                    << " (step length: " << stepLength
                    << "),   cost2: " << cost2
                    << " (step length: " << newStepLength << ")" << std::endl;
        ...
        if (lineSearchSteps < 2) {
            ...
            std::stringstream line_ss;
            if (logger) {
                line_ss << " Line-Search: Need to go to third order "
                           "approximation, looping... "
                        << std::endl;
                logger->logmessage(loglevel::debug, line_ss.str().c_str());
            }
            performLineSearch(
    ...
    void performLineSearch(
        ...
        for (int iStep = 2; iStep <= lineSearchSteps; ++iStep) {
            ...
            double alpha3 =
                (-b + std::sqrt(b * b - 3.0 * a * dirGradient)) / (3.0 * a);
    ...
    std::unique_ptr<ParameterUpdater> parameterUpdater =
    ...
    std::unique_ptr<LearningRateUpdater> learningRateUpdater =
        std::make_unique<LearningRateUpdater>(
            maxEpochs,
            learningRateInterp::linear);
};
...
std::unique_ptr<MinibatchOptimizer<BATCH_ELEMENT>>
getMinibatchOptimizer(OptimizationOptions const& options) {
    auto optim = std::make_unique<MinibatchOptimizer<BATCH_ELEMENT>>();

    options.for_each<MinibatchOptimizer<BATCH_ELEMENT>*>(
        setMinibatchOption, optim.get());

    return optim;
}

std::tuple<int, double, std::vector<double>>
runMinibatchOptimization(MinibatchOptimizationProblem<int>* problem);
...
template <typename T>
void clipToBounds(
    gsl::span<T const> lowerBounds,
    gsl::span<T const> upperBounds,
    gsl::span<T> x) {
    ...
    for (int i = 0;
         static_cast<typename gsl::span<T const>::index_type>(i) < x.size();
         ++i)

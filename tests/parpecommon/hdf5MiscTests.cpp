...
TEST_F(HDF5, OpenExistingFileNoOverwrite) {
    EXPECT_THROW(
        parpe::hdf5CreateFile(tempFileName, false), parpe::HDF5Exception);
}
...
TEST_F(HDF5, MutexGetLock) { parpe::hdf5MutexGetLock(); }
...
TEST_F(HDF5, ErrorStackWalker) {
    ...
    hid_t fileId = H5Fcreate(
        tempFileName.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
...
TEST_F(HDF5, CreateExistingGroup) {
    ...
    H5_SAVE_ERROR_HANDLER;
    EXPECT_THROW(
        parpe::hdf5CreateGroup(file, "/test", false), parpe::HDF5Exception);
    H5_RESTORE_ERROR_HANDLER;
}
...
TEST_F(HDF5, StringAttribute) {
    ...
    char const* attrName = "testA";
    char const* expAttrValue = "adsf";
    ...
    size_t size = 0;
    int ret = H5LTget_attribute_info(
        file.getId(), groupName, attrName, nullptr, &type_class, &size);
    EXPECT_TRUE(ret >= 0);
    char actValue[size];
...
TEST_F(HDF5, DatasetDimensions) {
    char const datasetName[] = "bla";
    int const rank = 3;
    hsize_t const dims[rank] = {1, 2, 3};
    int const buffer[6] = {1};

    EXPECT_FALSE(file.nameExists(datasetName));

    EXPECT_THROW(
        parpe::hdf5GetDatasetDimensions(
            file, datasetName, rank, nullptr, nullptr, nullptr, nullptr),
        H5::Exception);

    EXPECT_TRUE(
        H5LTmake_dataset_int(file.getId(), datasetName, rank, dims, buffer) >=
        0);
    ...
    parpe::hdf5GetDatasetDimensions(
        file, datasetName, rank, &d0, &d1, &d2, &d3);

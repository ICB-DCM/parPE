...
TEST(Testing, TenToMinusInf) { ASSERT_EQ(0.0, pow(10, -INFINITY)); }
...
TEST(Testing, CheckEqualArray) {
    double const expected[] = {1.0, 2.0, 3.0};
    double const actual[] = {1.0, 2.0, 3.0};
...
TEST(Common, Backtrace) {
    std::string output = captureStreamToString(
        []() { parpe::printBacktrace(5); }, stderr, STDERR_FILENO);
...
TEST(Common, FillArrayRandomDoubleIndividualInterval) {
    int const numTests = 100;
    double const min[numTests] = {-1.0, 0.0, 1.0};
    double const max[numTests] = {-0.5, 0.5, 1.5};
...
TEST(Logging, PrintDebugInfoAndWait) {
    captureStreamToString([]() { parpe::printDebugInfoAndWait(0); }, stdout);
}

TEST(Logging, MessageIsPrinted) {
    captureStreamToString(
        []() { parpe::logmessage(parpe::loglevel::error, "error"); }, stdout);
}

TEST(Logging, PrintMPIInfo) {
    std::string s =
        captureStreamToString([]() { parpe::printMPIInfo(); }, stdout);
...
TEST(Logging, LogProcessStats) {
    std::string s =
        captureStreamToString([]() { parpe::logProcessStats(); }, stdout);
...
TEST(CostFunction, MseZero) {
    ...
    std::vector<double*> predictionGradient = {
        &predictionGradient0[0], &predictionGradient0[1]};
...
TEST(CostFunction, LinearModel3) {
    ...
    std::vector<std::vector<double>> gradExp = {
        {4.0, 1.0, 1.0}, {8.0, 2.0, 1.0}};

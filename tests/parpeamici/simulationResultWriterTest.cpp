...
TEST(SimulationResultWriter, ResultWriter) {
    // setup ResultWriter
    char const* tmpName = "parpeTest_testResultWriter.h5";
    ...
    std::vector<double> const timepoints{1.0, 2.0};
    ...
    parpe::checkEqualArray(
        rdata.x.data(), xAct.data(), xAct.size(), 1e-16, 1e-16);

    auto yMesAct =
        amici::hdf5::getDoubleDataset2D(file, rw.yMesPath + "/1", m, n);
    parpe::checkEqualArray(
        measurements.data(), yMesAct.data(), yMesAct.size(), 1e-16, 1e-16);
}

TEST(SimulationResultWriter, ResultWriterNewExistingFile) {
    char const* tmpName = "parpeTest_testResultWriterNewExistingFile.h5";
    auto _ = gsl::finally([tmpName] { remove(tmpName); });

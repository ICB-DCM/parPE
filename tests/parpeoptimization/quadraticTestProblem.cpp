...
QuadraticTestProblem::QuadraticTestProblem(std::unique_ptr<Logger> logger)
    ...
    reporter = new NiceMock<OptimizationReporterMock>(
        cost_fun_.get(), std::make_unique<Logger>());
}
...
QuadraticTestProblem::getReporter() const {
...
QuadraticOptimizationMultiStartProblem::getLocalProblem(
    ...
    auto loggerPrefix =
        std::string("[start ") + std::to_string(multiStartIndex) + "]";
...
bool OptimizationReporterTest::starting(
...
bool OptimizationReporterTest::iterationFinished(
    gsl::span<double const> /*parameters*/,
...
bool OptimizationReporterTest::beforeCostFunctionCall(
    gsl::span<double const> /*parameters*/) const {
...
bool OptimizationReporterTest::afterCostFunctionCall(
    gsl::span<double const> parameters,
    double objectiveFunctionValue,
    gsl::span<double const> objectiveFunctionGradient) const {
...
void OptimizationReporterTest::finished(
    double /*optimalCost*/,
    gsl::span<double const> /*parameters*/,
    int exitStatus) const {
    //    mock().actualCall("OptimizationReporterTest::finished").withIntParameter("exitStatus",
    //    exitStatus);
}

...
TEST(localOptimizationToms611, testOptimizationGetlocalOptimum) {
    ...
    mock()
        ...
        .withIntParameter("exitStatus", x_and_relative_function_convergence);
...
TEST(localOptimizationToms611, testReporterCalled) {
    int const allowedLineSearchSteps = 3; // only true when iteration limit == 1
    parpe::QuadraticTestProblem problem;
    auto o = problem.getOptimizationOptions();
    o.maxOptimizerIterations = 1;
    o.setOption(
        "mxfcal",
        allowedLineSearchSteps + 1); // +1 for initial function evaluation
    ...
    // others
    mock().expectNCalls(
        o.maxOptimizerIterations * allowedLineSearchSteps + 1,
        "OptimizationReporterTest::beforeCostFunctionCall");
    mock().expectNCalls(
        o.maxOptimizerIterations * allowedLineSearchSteps, "testObj");

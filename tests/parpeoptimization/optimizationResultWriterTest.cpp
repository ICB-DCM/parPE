...
TEST(OptimizationResultWriter, ResultWriter) {
    char const* tmpFilename = "deleteme.h5";
    ...
    w.logOptimizerIteration(
        1, gsl::span<double>(), 0.0, gsl::span<double>(), 1.0, 2.0);

    // should it be possible to have the same iteration twice?
    w.logOptimizerIteration(
        1, gsl::span<double>(), 0.0, gsl::span<double>(), 1.0, 2.0);

    w.logObjectiveFunctionEvaluation(
        gsl::span<double>(), 1.0, gsl::span<double>(), 1, 2, 3.0);

...
TEST(MinibatchOptimization, CreatesBatches) {
    ...
    EXPECT_TRUE(
        std::vector<int>(input.begin(), input.begin() + batchSize) ==
        batchesAct[0]);
    EXPECT_TRUE(
        std::vector<int>(input.begin() + batchSize, input.end()) ==
        batchesAct[1]);
...
TEST(MinibatchOptimization, UpdatesParameters) {
    ...
    for (int i = 0; i < numParameters; i++)
        if (!errored)
            if (parametersExp[i] > toleratedError or
                parametersExp[i] < -toleratedError)
                errored = true;
...
class MinibatchOptimizationLinearModel : public ::testing::Test {
  ...
  protected:
    ...
    void generateRandomFeatures() {
        ...
        std::mt19937 rng(rd());
        data.assign(
            numDatasets, std::vector<double>(trueParameters.size() - 1));
        for (int i = 0; i < numDatasets; ++i) {
            std::generate(data[i].begin(), data[i].end(), [&unif, &rng]() {
                return unif(rng);
            });
            // std::cout<<data[i];
        }
    ...
    std::unique_ptr<parpe::LinearModelMSE> getLinearModelMSE() {
        ...
        auto lm2 =
            std::make_unique<parpe::LinearModelMSE>(trueParameters.size());
        lm2->datasets = data;
        lm2->labels = labels;
        return lm2;
    }
...
TEST_F(
    ...
    for (int i = 0; i < numDatasets; ++i) {
        ...
        EXPECT_EQ(0.0, mse);
        EXPECT_TRUE(
            std::vector<double>(trueParameters.size(), 0.0) == gradient);
    }
}
...
TEST_F(MinibatchOptimizationLinearModel, MinibatchSucceedFromOptimum) {
    ...
    auto result = mb.optimize(
        *lm2,
        ...
        nullptr);
    EXPECT_EQ(
        (int)parpe::minibatchExitStatus::gradientNormConvergence,
        std::get<0>(result));
...
TEST_F(MinibatchOptimizationLinearModel, LinearModel) {
    // optimization/tests/unittests_optimization -sg
    // minibatchOptimizationLinearModel -sn linearModel
    ...
    mb.maxEpochs = 100;
    // mb.parameterUpdater =
    // std::make_unique<parpe::ParameterUpdaterVanilla>(0.02);

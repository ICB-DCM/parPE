...
TEST(optimizationResultWriter, testResultWriter) {
    ...
    w.logOptimizerIteration(1, gsl::span<double>(), 0.0, gsl::span<double>(), 1.0, 2.0, true);
    // should it be possible to have the same iteration twice?
    w.logOptimizerIteration(1, gsl::span<double>(), 0.0, gsl::span<double>(), 1.0, 2.0, true);

    w.logObjectiveFunctionEvaluation(gsl::span<double>(), 1.0, gsl::span<double>(), 1, 2, 3.0, true, true);

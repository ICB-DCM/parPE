 ...
 * robust towards disadvantagous regions in parameter space.
  */
FunctionEvaluationStatus MinibatchOptimizer::rescueInterceptor(gsl::span<double> parameters,
															   gsl::span<const double> oldParameters,
															   gsl::span<double> gradient,
															   gsl::span<const double> oldGradient,
															   double &cost,
															   int &subsequentFails,
															   ...
															   OptimizationReporter *reporter)
{
	...
	while(status == functionEvaluationFailure) {
		...
		auto status = evaluate(f, parameters, datasets, cost, gradient, batchLogger.get(), reporter);
	}

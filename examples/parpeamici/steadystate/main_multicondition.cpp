...
/**
 * @brief The SteadystateApplication class subclasses
 * parpe::OptimizationApplication which provides a frame for a standalone
 * program to solve a multi-start local optimization problem.
 */
...
class SteadystateApplication : public parpe::OptimizationApplication {
  public:
    ...
    void initProblem(
        ...
        // The same file should only be opened/created once, an then only be
        // reopened
        ...
        auto optimizationOptions =
            parpe::OptimizationOptions::fromHDF5(dataProvider->getHdf5File());
...
/**
 * @brief The SteadystateLocalOptimizationApplication class overrides the
 * multi-start optimization in the base class and performs only a single
 * optimization run. This is mostly for debugging.
 */

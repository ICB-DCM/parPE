...
MultiConditionProblem::MultiConditionProblem(
    ...
    cost_fun_ =
        std::make_unique<SummedGradientFunctionGradientFunctionAdapter<int>>(
...
void MultiConditionProblem::fillInitialParameters(
...
MultiConditionProblem::getReporter() const {
...
MultiConditionProblemMultiStartOptimizationProblem ::
    ...
    , result_writer_(resultWriter)
    , load_balancer_(loadBalancer)
    , logger_(std::move(logger)) {}

int MultiConditionProblemMultiStartOptimizationProblem::getNumberOfStarts()
    const {
    return options_.numStarts;
}

bool MultiConditionProblemMultiStartOptimizationProblem::restartOnFailure()
    const {
    return options_.retryOptimization;
}

std::unique_ptr<OptimizationProblem>
MultiConditionProblemMultiStartOptimizationProblem::getLocalProblem(
...
void printSimulationResult(
    ...
    if (!rdata) {
        ...
        logger->logmessage(
            loglevel::error, "AMICI simulation failed unexpectedly.");
        return;
    }
    ...
    if (rdata->sensi >= amici::SensitivityOrder::first &&
        ...
        rdata->sensi >= amici::SensitivityOrder::first &&
        rdata->sensi_meth == amici::SensitivityMethod::forward) {
        sensi_mode = 'F';
    }
    bool with_sensi = rdata->sensi >= amici::SensitivityOrder::first &&
                      rdata->sensi_meth != amici::SensitivityMethod::none;

    logger->logmessage(
        loglevel::debug,
        "Result for %d: %g (%d) (%d/%d/%.4fs/%c%d)",
        jobId,
        rdata->llh,
        rdata->status,
        rdata->numsteps.empty() ? -1
    ...
    if (with_sensi) {
        for (std::vector<amici::realtype>::size_type i = 0;
             i < rdata->sllh.size();
             ++i) {
            if (std::isnan(rdata->sllh[i])) {
                logger->logmessage(
                    loglevel::debug, "Gradient contains NaN at %d", i);
                break;
            }

            if (std::isinf(rdata->sllh[i])) {
                logger->logmessage(
                    loglevel::debug, "Gradient contains Inf at %d", i);
                break;
            }
        }
    }
...
void saveSimulation(
    ...
    if (gradient.size() == parameters.size()) {
        ...
        if (!gradient.empty()) {
            std::copy_n(
                gradient.begin(),
                std::min(gradient.size(), parameters.size()),
                dummyGradient);
        }
        hdf5CreateOrExtendAndWriteToDouble2DArray(
            file,
            fullGroupPath,
            "simulationLogLikelihoodGradient",
    ...
    //            file.getId(), fullGroupPath, "simulationStateSensitivities",
    //            stateSensi, stateSensi.size() / parameters.size(),
    //            parameters.size());
    ...
    hdf5CreateOrExtendAndWriteToInt2DArray(
        ...
        fullGroupPath,
        "simulationStatus",
        gsl::make_span<int const>(&status, 1));

    hdf5CreateOrExtendAndWriteToString1DArray(
        file, fullGroupPath, "simulationLabel", label);
...
AmiciSimulationRunner::AmiciResultPackageSimple runAndLogSimulation(
    ...
    for (int trial = 1; trial <= maxNumTrials; ++trial) {
        ...
        if (rdata) {
            ...
            bool backwardFailed = solver->getSensitivityOrder() >=
                                      amici::SensitivityOrder::first &&
                                  solver->getSensitivityMethod() ==
                                      amici::SensitivityMethod::adjoint &&
                                  !rdata->sllh.empty() &&
                                  std::isnan(rdata->sllh[0]);
        ...
        try {
            rdata = run_amici_simulation(
                *solver, edata.get(), model, false, logger);
    ...
    if (resultWriter && rdata &&
        (solverTemplate.getSensitivityOrder() > amici::SensitivityOrder::none ||
         logLineSearch)) {
        saveSimulation(
            resultWriter->getH5File(),
            resultWriter->getRootPath(),
            model.getParameters(),
            rdata->llh,
            rdata->sllh,
            timeSeconds,
    ...
    if (rdata) {
        return AmiciSimulationRunner::AmiciResultPackageSimple{
            ...
            (solverTemplate.getSensitivityOrder() >
             ...
             solverTemplate.getSensitivityMethod() !=
                 amici::SensitivityMethod::none)
                ? rdata->sllh
                : std::vector<double>(),
            rdata->y,
    ...
    return AmiciSimulationRunner::AmiciResultPackageSimple{
        ...
        timeSeconds,
        (solverTemplate.getSensitivityOrder() > amici::SensitivityOrder::none)
            ? std::vector<double>(model.nplist(), NAN)
            : std::vector<double>(),
        std::vector<double>(model.nytrue, NAN),
        std::vector<double>(model.nytrue, NAN),
        sendStates ? std::vector<double>(model.nx_rdata, NAN)
                   : std::vector<double>(),
...
FunctionEvaluationStatus getModelOutputsAndSigmas(
    ...
    auto parameterVector =
        std::vector<double>(parameters.begin(), parameters.end());
    auto jobFinished = [&errors, &modelOutputs, &modelSigmas](
    ...
    if (loadBalancer && loadBalancer->isRunning()) {
        ...
        errors += simRunner.runSharedMemory(
            [&dataProvider, &resultWriter, logLineSearch, &sendStates](
...
void messageHandler(
    ...
    auto workPackage =
        amici::deserializeFromChar<AmiciSummedGradientFunction::WorkPackage>(
    ...
    for (auto conditionIdx : workPackage.conditionIndices) {
        dataProvider->updateSimulationParametersAndScale(
            conditionIdx, workPackage.optimizationParameters, *model);
        Logger logger(
            workPackage.logPrefix + "c" + std::to_string(conditionIdx));
...
FunctionEvaluationStatus AmiciSummedGradientFunction::evaluate(
    ...
    *objectiveFunctionValue = 1;
    ...
    int errors =
        runSimulations(parameters, fval, gradient, datasets, logger, cpuTime);
...
AmiciSummedGradientFunction::getAllMeasurements() const {
    return dataProvider->getAllMeasurements();
}

void AmiciSummedGradientFunction::messageHandler(
    std::vector<char>& buffer,
    int jobId) const {
    parpe::messageHandler(
        dataProvider, resultWriter, logLineSearch, buffer, jobId, sendStates);
}

amici::ParameterScaling
AmiciSummedGradientFunction::getParameterScaling(int parameterIndex) const {
...
int AmiciSummedGradientFunction::runSimulations(
    ...
    auto parameterVector = std::vector<double>(
        optimizationParameters.begin(), optimizationParameters.end());
    double simulationTimeSec = 0.0;

    AmiciSimulationRunner simRunner(
        parameterVector,
        !objectiveFunctionGradient.empty() ? amici::SensitivityOrder::first
                                           : amici::SensitivityOrder::none,
        dataIndices,
        [&nllh,
         &objectiveFunctionGradient,
         ...
         &errors,
         this](JobData* job, int /*jobIdx*/) {
            errors += this->aggregateLikelihood(
                *job,
                nllh,
                objectiveFunctionGradient,
                simulationTimeSec,
...
int AmiciSummedGradientFunction::aggregateLikelihood(
    ...
    // deserialize
    auto results = amici::deserializeFromChar<ResultMap>(
    ...
    for (auto const& result : results) {
        ...
        if (!negLogLikelihoodGradient.empty()) {
            ...
            dataProvider->mapAndSetOptimizationToSimulationVariables(
                conditionIdx, optimizationParameters, p, scaleOpt, scaleSim);
            addSimulationGradientToObjectiveFunctionGradient(
                conditionIdx,
                resultPackage.gradient,
...
void AmiciSummedGradientFunction::
    addSimulationGradientToObjectiveFunctionGradient(
        int conditionIdx,
        gsl::span<double const> simulationGradient,
        gsl::span<double> objectiveFunctionGradient,
        gsl::span<double const> simulationParameters) const {
...
void AmiciSummedGradientFunction::setSensitivityOptions(

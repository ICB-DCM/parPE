...
int MultiConditionDataProviderHDF5::getNumberOfSimulationConditions() const {
    ...
    hdf5GetDatasetDimensions(
        file_, hdf5_reference_condition_path_, 2, &d1, &d2);
...
void MultiConditionDataProviderHDF5::
    ...
    for (std::vector<int>::size_type i_plist = 0; i_plist < plist.size();
         ++i_plist) {
        ...
        if (mapping[i] >= 0) {
            ...
            simulation[i] = getScaledParameter(
                getUnscaledParameter(
                    optimization[mapping[i]], optimizationScale[mapping[i]]),
                simulationScale[i]);
...
void MultiConditionDataProviderHDF5::updateFixedSimulationParameters(
    ...
    if (conditionIdxPreeq >= 0) {
        ...
        readFixedSimulationParameters(
            conditionIdxPreeq, edata.fixedParametersPreequilibration);
...
void MultiConditionDataProviderHDF5::setModel(
    std::unique_ptr<amici::Model> model) {
    model_ = std::move(model);
}

std::vector<std::string>
MultiConditionDataProviderHDF5::getProblemParameterIds() const {
...
void MultiConditionDataProviderHDF5::readFixedSimulationParameters(
    ...
    hdf5Read2DDoubleHyperslab(
        file_, hdf5_condition_path_, model_->nk(), 1, 0, conditionIdx, buffer);
...
MultiConditionDataProviderHDF5::getExperimentalDataForCondition(
    ...
    edata->setTimepoints(amici::hdf5::getDoubleDataset1D(
        file_,
        root_path_ + "/measurements/t/" + std::to_string(simulationIdx)));
...
std::vector<int> MultiConditionDataProviderHDF5::getSensitivityParameterList(
    std::vector<int> const& mapping) const {
    ...
    int nplist = std::count_if(
        mapping.begin(), mapping.end(), [](int i) { return i >= 0; });
    int i_plist = 0;
...
std::vector<double> MultiConditionDataProviderHDF5::getSigmaForSimulationIndex(
    int simulationIdx) const {
    hsize_t dim1, dim2;
    [[maybe_unused]] auto lock = hdf5MutexGetLock();
...
MultiConditionDataProviderHDF5::getMeasurementForSimulationIndex(
    ...
    return amici::hdf5::getDoubleDataset2D(
        file_,
        hdf5_measurement_path_ + "/" + std::to_string(simulationIdx),
        dim1,
        dim2);
}
...
std::vector<int> MultiConditionDataProviderHDF5::getReinitializationIndices(
    ...
    hsize_t const len = 1;
    hsize_t const offset = simulationIdx;
...
void MultiConditionDataProviderHDF5::checkDataIntegrity() const {
    ...
    RELEASE_ASSERT(
        dataParameterIds == modelParameterIds, "Parameter IDs do not match.");

    std::string speciesIdsPath = root_path_ + "/model/stateIds";
    auto dataSpeciesIds = hdf5Read1dStringDataset(file_, speciesIdsPath);
    RELEASE_ASSERT(
        dataSpeciesIds == model_->getStateIds(), "State IDs do not match.");
    ...
    RELEASE_ASSERT(
        fixedParIds == model_->getFixedParameterIds(),
    ...
    RELEASE_ASSERT(
        observableIds == model_->getObservableIds(),
...
amici::ParameterScaling MultiConditionDataProviderDefault::getParameterScaleOpt(
...
void MultiConditionDataProviderDefault::updateSimulationParametersAndScale(
    int /*conditionIndex*/,
    gsl::span<double const> optimizationParams,
    amici::Model& model) const {
    logmessage(
    ...
    model.setParameters(std::vector<double>(
        optimizationParams.begin(), optimizationParams.end()));
}
...
std::vector<std::string>
MultiConditionDataProviderDefault::getProblemParameterIds() const {
    // not implemented
    std::terminate();
}

double applyChainRule(
    double gradient,
    double parameter,

...
int AmiciSimulationRunner::runDistributedMemory(
    ...
    auto numJobsTotal = static_cast<int>(std::ceil(
        static_cast<double>(condition_indices_.size()) /
        maxSimulationsPerPackage));
    std::vector<JobData> jobs{
        static_cast<decltype(jobs)::size_type>(numJobsTotal)};
...
int AmiciSimulationRunner::runSharedMemory(
    const messageHandlerFunc& messageHandler,
    ...
    std::vector<JobData> jobs{
        static_cast<unsigned int>(condition_indices_.size())};
...
void AmiciSimulationRunner::queueSimulation(
    ...
    *d = JobData(jobDone, jobDoneChangedCondition, jobDoneChangedMutex);
    ...
    if (callback_job_finished_)
        d->callbackJobFinished =
            std::bind(callback_job_finished_, std::placeholders::_1, jobIdx);

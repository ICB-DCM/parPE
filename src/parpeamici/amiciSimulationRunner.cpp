...
int AmiciSimulationRunner::runDistributedMemory(LoadBalancerMaster *loadBalancer, const int maxSimulationsPerPackage)
{
    ...
    auto numJobsTotal = static_cast<int>(
                std::ceil(
                    static_cast<double>(condition_indices_.size())
    ...
    for (int jobIdx = 0; jobIdx < numJobsTotal; ++jobIdx) {
        int simulationsLeft = static_cast<int>(condition_indices_.size()) - numConditionsSent;
        int simulationsCurrentPackage = std::min(simulationsLeft, maxSimulationsPerPackage);

        auto currentConditions = std::vector<int>(
                    &condition_indices_[static_cast<std::vector<int>::size_type>(numConditionsSent)],
        ...
        queueSimulation(loadBalancer, &jobs[jobIdx],
                        ...
                        jobIdx, optimization_parameters_, sensitivity_order_, currentConditions);
...
int AmiciSimulationRunner::runSharedMemory(const messageHandlerFunc& messageHandler, bool sequential)
{
    ...
    std::vector<JobData> jobs {static_cast<unsigned int>(condition_indices_.size())};
...
#endif
    for (int simulationIdx = 0; simulationIdx < (signed)condition_indices_.size(); ++simulationIdx) {
        ...
        AmiciWorkPackageSimple work {optimization_parameters_, sensitivity_order_, curConditionIndices, log_prefix_};
                                             ...
                                             std::vector<int> const& conditionIndices)
{
    ...
    *d = JobData(jobDone, jobDoneChangedCondition, jobDoneChangedMutex);
    ...
    AmiciWorkPackageSimple work {optimizationParameters, sensitivityOrder, conditionIndices, log_prefix_};

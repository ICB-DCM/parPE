...
void MultiStartOptimization::runMultiThreaded() const {
    ...
    num_threads =
        std::min(num_threads, static_cast<unsigned int>(numberOfStarts));
    ...
    for (int start_idx = 0; start_idx < numberOfStarts; ++start_idx) {
        futures.push_back(boost::asio::post(
            pool, std::packaged_task<std::pair<int, int>()>([this, start_idx] {
...
void MultiStartOptimization::runSingleThreaded() {
    ...
    while (true) {
        ...
        if (result) {
            logmessage(loglevel::debug, "Start #%d finished successfully", ms);
            ++numSucceeded;
        } else {
            logmessage(
                loglevel::debug,
...
int MultiStartOptimization::runStart(int start_idx) const {
    logmessage(loglevel::debug, "Starting local optimization #%d", start_idx);

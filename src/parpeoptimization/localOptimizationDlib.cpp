...
template <
    ...
    typename layout>
span<T const> make_span(
    dlib::matrix<T, num_rows, num_cols, mem_manager, layout> const& matrix) {
    return span<T const>(matrix.begin(), matrix.size());
}
...
template <
    ...
    typename layout>
span<T>
make_span(dlib::matrix<T, num_rows, num_cols, mem_manager, layout>& matrix) {
...
parpe::OptimizerDlibLineSearch::optimize(OptimizationProblem* problem) {
    ...
    double finalFVal = dlib::find_min_box_constrained(
        ...
        [&optimizationController](column_vector const& x) {
            // objective function
            double fval = NAN;
            optimizationController->evaluate(
                gsl::make_span(x), fval, gsl::span<double>(nullptr, 0));

...
void optimizationProblemGradientCheckMultiEps(
    ...
    optimizationProblemGradientCheckMultiEps(
        problem, parameterIndices, multi_eps);
}
    ...
    for (int curInd : parameterIndices) {
        ...
        for (double epsilon : multi_eps) {
            ...
            problem->cost_fun_->evaluate(
                gsl::span<double>(thetaTmp), ff, gsl::span<double>());

            thetaTmp[curInd] = theta[curInd] - epsilon;
            problem->cost_fun_->evaluate(
                gsl::span<double>(thetaTmp), fb, gsl::span<double>());
        ...
        logmessage(
            ll,
            "%-25s (%d) g: %12.6g  fd_c: %12.6g  |Δ/fd_c|: %.6e  "
            "|Δ|: %12.6g  ϵ: %12.6g ",
...
void optimizationProblemGradientCheck(
    ...
    for (int curInd : parameterIndices) {
        ...
        //        printf("%5d%s g: %12.6g fd_f: %12.6g (Δ%12.6g) fd_c: %12.6g
        //        (Δ%12.6g) fd_b: %12.6g (Δ%12.6g)",
        ...
        logmessage(
            ll,
            "%5d g: %12.6g  fd_c: %12.6g  Δ/ff: %.6e  f: %12.6g",
            curInd,
            curGrad,
            fd_c,
            regRelError,
...
void OptimizationProblem::setOptimizationOptions(
    OptimizationOptions const& options) {
...
void OptimizationProblem::fillInitialParameters(
    ...
    fillArrayRandomDoubleIndividualInterval(
        parametersMin, parametersMax, buffer);
}
...
FunctionEvaluationStatus OptimizationReporter::evaluate(
    ...
    if (gradient.data()) {
        if (!have_cached_gradient_ || !std::equal(
                                          parameters.begin(),
                                          parameters.end(),
        ...
        if (!have_cached_cost_ || !std::equal(
                                      parameters.begin(),
                                      parameters.end(),
                                      cached_parameters_.begin())) {
            // Have to compute anew
...
void OptimizationReporter::printObjectiveFunctionFailureMessage() const {
    if (logger_)
        logger_->logmessage(
            loglevel::error, "Objective function evaluation failed!");
}

bool OptimizationReporter::starting(
    gsl::span<double const> initialParameters) const {
    ...
    logger_->setPrefix(
        default_logger_prefix_ + "i" + std::to_string(num_iterations_));

    return false;
}

bool OptimizationReporter::iterationFinished(
    gsl::span<double const> parameters,
    ...
    if (logger_)
        logger_->logmessage(
            loglevel::info,
            "iter: %d cost: %g time_iter: wall: %gs cpu: %gs time_optim: wall: "
            "%gs cpu: %gs",
            num_iterations_,
    ...
    if (result_writer_)
        result_writer_->logOptimizerIteration(
            num_iterations_,
            parameters.empty() ? cached_parameters_ : parameters,
            objectiveFunctionValue,
            // This might be misleading, the gradient could evaluated at other
            // parameters if there was a line search inbetween
            objectiveFunctionGradient.empty() ? cached_gradient_
...
bool OptimizationReporter::beforeCostFunctionCall(
...
bool OptimizationReporter::afterCostFunctionCall(
    gsl::span<double const> parameters,
    double objectiveFunctionValue,
    gsl::span<double const> objectiveFunctionGradient) const {
...
void OptimizationReporter::finished(
    double optimalCost,
    gsl::span<double const> parameters,
    int exitStatus) const {
    double timeElapsed = wall_timer_.getTotal();

    if ((optimalCost <= cached_cost_ || cached_parameters_.empty()) &&
        !parameters.empty()) {
        ...
        // get the optimal parameters from the optimizer. since we don't know
        // them, rather set to nan
        if (logger_)
            logger_->logmessage(
                loglevel::info,
                "cachedCost != optimalCost && parameters.empty()");
        cached_parameters_.assign(
            cached_parameters_.size(),
    ...
    if (logger_)
        logger_->logmessage(
            loglevel::info,
            "Optimizer status %d, final llh: %e, "
            "time: wall: %f cpu: %f.",
            exitStatus,
            cached_cost_,
            timeElapsed,
    ...
    if (result_writer_)
        result_writer_->saveOptimizerResults(
            cached_cost_,
            cached_parameters_,
            timeElapsed,
            cpu_time_total_sec_,
...
double OptimizationReporter::getFinalCost() const { return cached_cost_; }

std::vector<double> const& OptimizationReporter::getFinalParameters() const {
    return cached_parameters_;
}

void OptimizationReporter::setGradientFunction(
...
void OptimizationProblemImpl::fillParametersMin(
    gsl::span<double> buffer) const {
    std::copy(parametersMin.begin(), parametersMin.end(), buffer.begin());
}

void OptimizationProblemImpl::fillParametersMax(
    gsl::span<double> buffer) const {
    std::copy(parametersMax.begin(), parametersMax.end(), buffer.begin());
}

void OptimizationProblemImpl::setParametersMin(
    std::vector<double> parametersMin) {
    this->parametersMin = std::move(parametersMin);
}

void OptimizationProblemImpl::setParametersMax(
    std::vector<double> parametersMax) {
    this->parametersMax = std::move(parametersMax);
}

void OptimizationProblemImpl::setInitialParameters(
    std::vector<double> initial) {
    parametersStart = std::move(initial);
}

void OptimizationProblemImpl::fillInitialParameters(
    gsl::span<double> buffer) const {
    if (!parametersStart.empty()) {
        std::copy(
            parametersStart.begin(), parametersStart.end(), buffer.begin());
    } else {

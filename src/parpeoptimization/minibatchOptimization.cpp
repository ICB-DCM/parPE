...
void setMinibatchOption(
    ...
    if (key == "maxEpochs") {
        ...
        if (val == "Vanilla") {
            // already default optimizer->parameterUpdater =
            // std::make_unique<ParameterUpdaterVanilla>();
        } else if (
            val == "RmsProp" && !dynamic_cast<ParameterUpdaterRmsProp*>(
                                    optimizer->parameterUpdater.get())) {
            // this might have been set previously if there was an
            // updater-specific option before
            optimizer->parameterUpdater =
                std::make_unique<ParameterUpdaterRmsProp>();
        } else if (
            val == "Adam" && !dynamic_cast<ParameterUpdaterAdam*>(
                                 optimizer->parameterUpdater.get())) {
            // this might have been set previously if there was an
            // updater-specific option before
            optimizer->parameterUpdater =
                std::make_unique<ParameterUpdaterAdam>();
        } else if (
            val == "AdamClassic" && !dynamic_cast<ParameterUpdaterAdamClassic*>(
                                        optimizer->parameterUpdater.get())) {
            // this might have been set previously if there was an
            // updater-specific option before
            optimizer->parameterUpdater =
                std::make_unique<ParameterUpdaterAdamClassic>();
        } else {
            logmessage(
                loglevel::warning,
                "Ignoring unknown Minibatch parameterUpdater %s.",
                val.c_str());
        }
        ...
        if (val == "linear") {
            ...
            optimizer->learningRateUpdater =
                std::make_unique<LearningRateUpdater>(
                    optimizer->maxEpochs,
                    parpe::learningRateInterp::inverseLinear);
        } else if (val == "logarithmic") {
            optimizer->learningRateUpdater =
                std::make_unique<LearningRateUpdater>(
                    optimizer->maxEpochs,
                    parpe::learningRateInterp::logarithmic);
        }
...
runMinibatchOptimization(MinibatchOptimizationProblem<int>* problem) {
    auto minibatchOptimizer =
        getMinibatchOptimizer<int>(problem->getOptimizationOptions());
    ...
    return minibatchOptimizer->optimize(
        *costFun,
        data,
        initialParameters,
...
LearningRateUpdater::LearningRateUpdater(
    int maxEpochs,
...
void LearningRateUpdater::updateLearningRate(int currentEpoch) {
    ...
    // Depending on the interpolation mode the current learning rate computed
    // must be...
    if (learningRateInterpMode == learningRateInterp::linear) {
        currentLearningRate =
            startLearningRate - (startLearningRate - endLearningRate) *
                                    ((double)currentEpoch) /
                                    ((double)maxEpochs - 1);
    } else if (learningRateInterpMode == learningRateInterp::inverseLinear) {
        currentLearningRate = 1 / startLearningRate -
                              (1 / startLearningRate - 1 / endLearningRate) *
                                  ((double)currentEpoch) /
                                  ((double)maxEpochs - 1);
        ...
        currentLearningRate = log(startLearningRate) -
                              (log(startLearningRate) - log(endLearningRate)) *
                                  ((double)currentEpoch) /
                                  ((double)maxEpochs - 1);
...
void ParameterUpdaterRmsProp::updateParameters(
    double learningRate,
    int /*iteration*/,
    gsl::span<double const> gradient,
    gsl::span<double> parameters,
    gsl::span<double const> lowerBounds,
    ...
    for (int i = 0; i < numParameters; ++i) {
        ...
        parameters[i] += -learningRate * gradient[i] /
                         (std::sqrt(gradientNormCache[i]) + delta);
    }
...
void ParameterUpdaterRmsProp::undoLastStep() {
    // The cached gradient norm needs to be restored, since the new one is
    // probably NaN
...
void ParameterUpdaterMomentum::updateParameters(
    double learningRate,
    int /*iteration*/,
    gsl::span<double const> gradient,
    gsl::span<double> parameters,
...
void ParameterUpdaterAdam::updateParameters(
    double learningRate,
    int iteration,
    gsl::span<double const> gradient,
    gsl::span<double> parameters,
    ...
    for (int i = 0; i < numParameters; ++i) {
        ...
        gradientNormCache[i] =
            decayRateGradientNorm * gradientNormCache[i] +
            (1 - decayRateGradientNorm) * gradient[i] * gradient[i];

        tmpNumerator = gradientCache[i] /
                       (1 - std::pow(decayRateGradient, (double)iteration));
        tmpDenominator =
            std::sqrt(
                gradientNormCache[i] /
                (1 - std::pow(decayRateGradientNorm, (double)iteration))) +
            delta;
...
void ParameterUpdaterAdamClassic::updateParameters(
    double learningRate,
    int iteration,
    gsl::span<double const> gradient,
    gsl::span<double> parameters,
...
void ParameterUpdaterVanilla::updateParameters(
    double learningRate,
    int /*iteration*/,
    gsl::span<double const> gradient,
    gsl::span<double> parameters,
    ...
    for (int i = 0; i < numParameters; ++i) {
        // logmessage(LOGLVL_DEBUG, "p_%d: %f - %f = %f", i, parameters[i],
        // learningRate * gradient[i], parameters[i] - learningRate *
        // gradient[i]);
        parameters[i] -=
            learningRate * gradient[i] / (getVectorNorm(gradient) + delta);
    }

    clipToBounds(lowerBounds, upperBounds, parameters);
}

void ParameterUpdaterVanilla::initialize(
    [[maybe_unused]] unsigned int numParameters) {}

...
int main(int argc, char** argv) {
    ...
    MultiConditionProblem problem{
        &dataProvider, nullptr, nullptr, std::move(rw)};
    ...
    std::for_each(
        gradient.begin(), gradient.end(), [](double& d) { d = std::fabs(d); });
    auto maxAbsGradient = *std::max_element(gradient.begin(), gradient.end());
    std::for_each(gradient.begin(), gradient.end(), [fval](double& d) {
        d /= std::fabs(fval);
    });
    auto maxRelGradient = *std::max_element(gradient.begin(), gradient.end());
    parpe::logmessage(
        loglevel::info, "Max(abs(grad)) = " + std::to_string(maxAbsGradient));
    parpe::logmessage(
        loglevel::info,
        "Max(abs(grad)/fval) = " + std::to_string(maxRelGradient));

...
UserData getUserData() {
    return UserData(5, 3, 3, 4, 3, 3, 0, 0, 0, 1, 2, 2, 1, 9, 2, 2,
                    AMICI_SCALING_LOG10, AMICI_O2MODE_NONE);
}
...
int wrap_binit(void *cvode_mem, int which, N_Vector xB, N_Vector dxB,
               realtype t) {
...
int wrap_SensInit1(void *cvode_mem, N_Vector *sx, N_Vector *sdx,
                   void *user_data) {
    UserData *udata = (UserData *)user_data;
    return CVodeSensInit1(cvode_mem, udata->nplist, udata->sensi_meth,
                          sxdot_model_steadystate, sx);
}
...
int fdx0(N_Vector x0, N_Vector dx0, void *user_data) { return 0; }
...
int fJB(long int NeqBdot, realtype t, N_Vector x, N_Vector xB, N_Vector xBdot,
        ...
        N_Vector tmp3B) {
    return JB_model_steadystate(NeqBdot, t, x, xB, xBdot, JB, user_data, tmp1B,
                                tmp2B, tmp3B);
}
...
int frz(realtype t, int ie, N_Vector x, void *user_data, TempData *tdata,
        ReturnData *rdata) {
    return rz_model_steadystate(t, ie, x, user_data, tdata, rdata);
}

int fsrz(realtype t, int ie, N_Vector x, N_Vector *sx, void *user_data,
         TempData *tdata, ReturnData *rdata) {
    return srz_model_steadystate(t, ie, x, sx, user_data, tdata, rdata);
}

int fstau(realtype t, int ie, N_Vector x, N_Vector *sx, void *user_data,
          TempData *tdata) {
...
int fz(realtype t, int ie, N_Vector x, void *user_data, TempData *tdata,
       ReturnData *rdata) {
    return z_model_steadystate(t, ie, x, user_data, tdata, rdata);
}

int fsz(realtype t, int ie, N_Vector x, N_Vector *sx, void *user_data,
        TempData *tdata, ReturnData *rdata) {
...
int fdeltasx(realtype t, int ie, N_Vector x, N_Vector xdot, N_Vector xdot_old,
             N_Vector *sx, void *user_data, TempData *tdata) {
    return deltasx_model_steadystate(t, ie, x, xdot, xdot_old, sx, user_data,
                                     tdata);
}

int fdeltaxB(realtype t, int ie, N_Vector x, N_Vector xB, N_Vector xdot,
             N_Vector xdot_old, void *user_data, TempData *tdata) {
    return deltaxB_model_steadystate(t, ie, x, xB, xdot, xdot_old, user_data,
                                     tdata);
}
...
int fJy(realtype t, int it, N_Vector x, void *user_data, TempData *tdata,
        const ExpData *edata, ReturnData *rdata) {
    return Jy_model_steadystate(t, it, x, user_data, tdata, edata, rdata);
}

int fJz(realtype t, int ie, N_Vector x, void *user_data, TempData *tdata,
        const ExpData *edata, ReturnData *rdata) {
    return Jz_model_steadystate(t, ie, x, user_data, tdata, edata, rdata);
}

int fJrz(realtype t, int ie, N_Vector x, void *user_data, TempData *tdata,
         const ExpData *edata, ReturnData *rdata) {
    return Jrz_model_steadystate(t, ie, x, user_data, tdata, edata, rdata);
}

int fdJydy(realtype t, int it, N_Vector x, void *user_data, TempData *tdata,
           const ExpData *edata, ReturnData *rdata) {
    return dJydy_model_steadystate(t, it, x, user_data, tdata, edata, rdata);
}

int fdJydsigma(realtype t, int it, N_Vector x, void *user_data, TempData *tdata,
               const ExpData *edata, ReturnData *rdata) {
    return dJydsigma_model_steadystate(t, it, x, user_data, tdata, edata,
                                       rdata);
}

int fdJzdz(realtype t, int ie, N_Vector x, void *user_data, TempData *tdata,
           const ExpData *edata, ReturnData *rdata) {
    return dJzdz_model_steadystate(t, ie, x, user_data, tdata, edata, rdata);
}

int fdJzdsigma(realtype t, int ie, N_Vector x, void *user_data, TempData *tdata,
               const ExpData *edata, ReturnData *rdata) {
    return dJzdsigma_model_steadystate(t, ie, x, user_data, tdata, edata,
                                       rdata);
}

int fdJrzdz(realtype t, int ie, N_Vector x, void *user_data, TempData *tdata,
            const ExpData *edata, ReturnData *rdata) {
    return dJrzdz_model_steadystate(t, ie, x, user_data, tdata, edata, rdata);
}

int fdJrzdsigma(realtype t, int ie, N_Vector x, void *user_data,
                TempData *tdata, const ExpData *edata, ReturnData *rdata) {
    return dJrzdsigma_model_steadystate(t, ie, x, user_data, tdata, edata,
                                        rdata);
}

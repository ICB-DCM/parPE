...
int dJydsigma_model_steadystate(realtype t, int it, N_Vector x, TempData *tdata,
    ...
    memset(tdata->dJydsigma, 0,
           sizeof(realtype) * model->nytrue * model->ny * model->nJ);
    status = w_model_steadystate(t, x, NULL, tdata);
    int iy;
    if (!amiIsNaN(edata->my[0 * udata->nt + it])) {
        iy = 0;
        tdata->dJydsigma[iy + (0 + 0 * 1) * model->nytrue] =
    ...
    if (!amiIsNaN(edata->my[1 * udata->nt + it])) {
        iy = 1;
        tdata->dJydsigma[iy + (0 + 1 * 1) * model->nytrue] =
    ...
    if (!amiIsNaN(edata->my[2 * udata->nt + it])) {
        iy = 2;
        tdata->dJydsigma[iy + (0 + 2 * 1) * model->nytrue] =

...
int J_model_steadystate(long int N, realtype t, N_Vector x, N_Vector xdot,
    ...
    J->data[1 + 0 * 3] = -udata->p[1] * x_tmp[1] + udata->p[0] * udata->dwdx[0];
    ...
    J->data[2 + 2 * 3] = -udata->k[3] - udata->dwdx[1];
    for (ix = 0; ix < 9; ix++) {
        if (amiIsNaN(J->data[ix])) {
            J->data[ix] = 0;
            if (!udata->nan_J) {
                warnMsgIdAndTxt("AMICI:mex:fJ:NaN",
                                "AMICI replaced a NaN value in Jacobian and "
        ...
        if (amiIsInf(J->data[ix])) {
            warnMsgIdAndTxt("AMICI:mex:fJ:Inf", "AMICI encountered an Inf "
                                                "value in Jacobian! Aborting "
                                                "simulation ... ");
            return (-1);
        }
    }

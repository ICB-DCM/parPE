...
int J_model_steadystate(long int N, realtype t, N_Vector x, N_Vector xdot,
                        ...
                        N_Vector tmp3);
int JB_model_steadystate(long int NeqBdot, realtype t, N_Vector x, N_Vector xB,
                         N_Vector xBdot, DlsMat JB, void *user_data,
                         N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3B);
int JBand_model_steadystate(long int N, long int mupper, long int mlower,
                            realtype t, N_Vector x, N_Vector xdot, DlsMat J,
...
int JSparse_model_steadystate(realtype t, N_Vector x, N_Vector xdot, SlsMat J,
                              void *user_data, N_Vector tmp1, N_Vector tmp2,
                              N_Vector tmp3);
int JSparseB_model_steadystate(realtype t, N_Vector x, N_Vector xB,
                               N_Vector xBdot, SlsMat JB, void *user_data,
                               N_Vector tmp1B, N_Vector tmp2B, N_Vector tmp3B);
...
int JvB_model_steadystate(N_Vector vB, N_Vector JvB, realtype t, N_Vector x,
                          N_Vector xB, N_Vector xBdot, void *user_data,
                          N_Vector tmpB);
int Jy_model_steadystate(realtype t, int it, N_Vector x, void *user_data,
                         TempData *tdata, const ExpData *edata,
                         ReturnData *rdata);
int Jz_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                         TempData *tdata, const ExpData *edata,
                         ReturnData *rdata);
...
int dJrzdz_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                             TempData *tdata, const ExpData *edata,
                             ReturnData *rdata);
int dJydsigma_model_steadystate(realtype t, int it, N_Vector x, void *user_data,
                                TempData *tdata, const ExpData *edata,
                                ReturnData *rdata);
int dJydy_model_steadystate(realtype t, int it, N_Vector x, void *user_data,
                            TempData *tdata, const ExpData *edata,
                            ReturnData *rdata);
int dJzdsigma_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                                TempData *tdata, const ExpData *edata,
                                ReturnData *rdata);
int dJzdz_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                            TempData *tdata, const ExpData *edata,
                            ReturnData *rdata);
int deltaqB_model_steadystate(realtype t, int ie, N_Vector x, N_Vector xB,
                              N_Vector qBdot, N_Vector xdot, N_Vector xdot_old,
                              void *user_data, TempData *tdata);
int deltasx_model_steadystate(realtype t, int ie, N_Vector x, N_Vector xdot,
                              N_Vector xdot_old, N_Vector *sx, void *user_data,
                              TempData *tdata);
int deltax_model_steadystate(realtype t, int ie, N_Vector x, N_Vector xdot,
                             N_Vector xdot_old, void *user_data,
                             TempData *tdata);
int deltaxB_model_steadystate(realtype t, int ie, N_Vector x, N_Vector xB,
                              N_Vector xdot, N_Vector xdot_old, void *user_data,
                              TempData *tdata);
int drzdp_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                            TempData *tdata);
int drzdx_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                            TempData *tdata);
...
int dwdp_model_steadystate(realtype t, N_Vector x, N_Vector dx,
                           void *user_data);
int dwdx_model_steadystate(realtype t, N_Vector x, N_Vector dx,
                           void *user_data);
int dxdotdp_model_steadystate(realtype t, N_Vector x, N_Vector dx,
                              void *user_data);
int dydp_model_steadystate(realtype t, int it, N_Vector x, void *user_data,
                           TempData *tdata);
int dydx_model_steadystate(realtype t, int it, N_Vector x, void *user_data,
                           TempData *tdata);
int dzdp_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                           TempData *tdata);
int dzdx_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                           TempData *tdata);
int qBdot_model_steadystate(realtype t, N_Vector x, N_Vector xB, N_Vector qBdot,
                            void *user_data);
int root_model_steadystate(realtype t, N_Vector x, realtype *root,
                           void *user_data);
int rz_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
...
int sx0_model_steadystate(N_Vector *sx0, N_Vector x, N_Vector dx,
                          void *user_data);
int sxdot_model_steadystate(int Ns, realtype t, N_Vector x, N_Vector xdot,
                            int ip, N_Vector sx, N_Vector sxdot,
                            void *user_data, N_Vector tmp1, N_Vector tmp2);
...
int xBdot_model_steadystate(realtype t, N_Vector x, N_Vector xB, N_Vector xBdot,
                            void *user_data);
int xdot_model_steadystate(realtype t, N_Vector x, N_Vector xdot,
                           void *user_data);
int y_model_steadystate(realtype t, int it, N_Vector x, void *user_data,
                        ReturnData *rdata);
int z_model_steadystate(realtype t, int ie, N_Vector x, void *user_data,
                        TempData *tdata, ReturnData *rdata);

...
MultiConditionDataProvider::MultiConditionDataProvider(UserData udata, const char *hdf5Filename) : modelDims(udata)
{
...
int MultiConditionDataProvider::getNumberOfConditions()
{
    ...
    hid_t dset = H5Dopen(file_id, "/measurements/y", H5P_DEFAULT);
...
{
    return AMI_HDF5_getIntScalarAttribute(file_id, "/parameters", "numConditionSpecificParameters");
}
...
int MultiConditionDataProvider::updateFixedSimulationParameters(int conditionIdx, UserData *udata)
{
    ...
    hdf5Read2DDoubleHyperslab(file_id, "/fixedParameters/k", udata->nk, 1, 0, conditionIdx, udata->k);
...
ExpData *MultiConditionDataProvider::getExperimentalDataForExperimentAndUpdateUserData(int conditionIdx, UserData *udata)
{
    ...
    edata->am_my = new double[udata->ny];
    edata->am_ysigma = new double[udata->ny];

    // no events
    edata->am_mz = 0; edata->am_zsigma = 0;

    hdf5Read2DDoubleHyperslab(file_id, "/measurements/y",      udata->ny, 1, 0, conditionIdx, edata->am_my);
    hdf5Read2DDoubleHyperslab(file_id, "/measurements/ysigma", udata->ny, 1, 0, conditionIdx, edata->am_ysigma);

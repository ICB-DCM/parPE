...
IGNORE_TEST(simulationWorkerAmici, testSerializeResultPackageMessage) {
    ...
    JobResultAmiciSimulation::serialize(&rdataExp, &udataExp, statusExp, buffer);
...
TEST(simulationWorkerAmici, testSerializeWorkPackageMessage) {
    ...
    // serialize
    int workPackageLength = JobAmiciSimulation::getLength(nTheta, expWp.lenData);
    ...
    for(int i = 0; i < nTheta; ++i) {
        DOUBLES_EQUAL(actWp.simulationParameters[i], expWp.simulationParameters[i], 0);
    }
    for(int i = 0; i < lenDataElements; ++i) {
